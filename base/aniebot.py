...
class aniebot(Client):
    ...
    async def start(self):
        LOGGER(__name__).info(f"Starting Anie Assistant Clients")
        if config.STRING1:
            await self.one.start()
            try:
                await self.one.join_chat("AnieBots")
                await self.one.join_chat("AniebotSupports")
            except:
                pass
            assistants.append(1)
            try:
